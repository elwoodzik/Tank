...
class Leya {
    ...
    preloaded() {
        const elements = Object.keys(this.preload()).length;
        ...
        AssetManager.load(this.preload(), this.loaded.bind(this), (loaded, total, key, path, success) => {
            console.log(loaded, total, key, path, success)
            bar.setStatusX(loaded)
            percentText.use(Math.round(loaded / elements * 100));
        });
    }
